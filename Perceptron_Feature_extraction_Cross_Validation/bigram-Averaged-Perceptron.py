...
This code shows bigram presentation and Averaged Perceptron
...
if __name__== "__main__":
    ...
    tr_bigram_sparse_matrix = bigram_vectorizer.fit_transform(text_tr)
    ...
    error_number2 = []
    for i in range(K):
        X_train, X_test = tr_bigram_sparse_matrix[tr_idx[i]], tr_bigram_sparse_matrix[te_idx[i]]
        ...
        error_number2.append(compare_result)
    
    error_number_linear_bigram = np.mean(np.array(error_number2))
    error_rate_linear_bigram = error_number_linear_bigram /1.0 /Y_test.shape[0]
    
    print "error_rate_linear_bigram", error_rate_linear_bigram
    fp = open("error_rate_linear_bigram.txt", 'w')
    fp.write('error_rate_linear_bigram')
    fp.write('\n')
    fp.write(str(error_rate_linear_bigram))
    fp.close()
