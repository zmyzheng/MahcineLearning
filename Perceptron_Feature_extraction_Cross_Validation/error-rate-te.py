...
This code shows error rate of test data by unigram presentation and Averaged Perceptron
...
if __name__== "__main__":
    ...
    label_te_matrix = (np.array([label_te]).T - 0.5) * 2
    ...
    X_test = te_unigram_sparse_matrix
    Y_train = label_tr_matrix
    Y_test = label_te_matrix
    ...
    error_number_te = len((linear_result.sign() - Y_test).nonzero()[0])
    error_rate_te = error_number_te /1.0 /Y_test.shape[0]
    
    print "error_rate_te", error_rate_te
    fp = open("error_number_te.txt", 'w')
    fp.write(str(error_rate_te))
    fp.write('error_rate_te')
    fp.write('\n')
    fp.close()
