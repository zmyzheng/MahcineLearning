...
This code shows tf-idf-modified presentation and Averaged Perceptron
...
if __name__== "__main__":
    ...
    # tf-idf-rariant presentation
    tfid_vectorizer_variant = TfidfVectorizer(min_df=1, sublinear_tf=True)
    tr_tfid_sparce_matrix_variant = tfid_vectorizer_variant.fit_transform(text_tr)
    ...
    error_number4 = []
    for i in range(K):
        X_train, X_test = tr_tfid_sparce_matrix_variant[tr_idx[i]], tr_tfid_sparce_matrix_variant[te_idx[i]]
        ...
        error_number4.append(compare_result)
    
    error_number_linear_tfid_modified = np.mean(np.array(error_number4))
    error_rate_linear_tfid_modified = error_number_linear_tfid_modified /1.0 /Y_test.shape[0]
    
   
    print "error_rate_linear_tfid_modified", error_rate_linear_tfid_modified
    fp = open("error_rate_linear_tfid_vmodified.txt", 'w')
    fp.write('error_rate_linear_tfid_modified')
    fp.write('\n')
    fp.write(str(error_rate_linear_tfid_modified))
    fp.close()
